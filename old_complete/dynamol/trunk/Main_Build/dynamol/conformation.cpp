...
/** First section is for conf thread stuff */
confThread::confThread(XmodelView *view, const char *name) 
: dynoThread(NULL) 
{
   setName(name); 
...
confThread::~confThread() {delete conf;}
int confThread::rtti() {return 4;}
void confThread::stopThread() {conf->stop = true;}
void confThread::setName(string name) {this->name = name;}
QString confThread::getName() {return this->name;};

void confThread::run() {
   cout <<"confThread::run: " << endl;
   conf->run();
   cout <<"Back from confThread::run"<<endl;
   //int test = 0; cin >> test;
   emit threadDone();
...
void confThread::setUp(list<molecule *> *molList, vector<bond *> &rotBonds, string outFile) {
   conf->setUp(molList, rotBonds, outFile);
}

void confThread::eSetup(bool min, float cutoff, int clusterNum, float rmse, float minE, float maxForce, int maxIter) {
   conf->eSetup(min, cutoff, clusterNum, rmse, minE, maxForce, maxIter);
}
...
void Conformation::run() {
  // int test = 0; cin >> test;
   ...
   string del = "del " + outFile;
   QFile f(outFile); f.remove();
   ...
   confNode *head = buildConfList(confList);
   energy eng;
   ...
   vector<nonBond *> nonBonds, frozenNb;
   vector<torsion *> torsions, frozenTor;
   ...
   for (int i=0; i<confList.size(); i++) {
      ...
      for (int j=0; j<confList[i]->angles.size(); j++) {
         bond *bd = rotBonds[j];
      ...
      if (i % 1000 == 0) {
         ...
         //string num2; num2.setNum(confList.size());
      ...
      if (confList[i]->energy < 100000.0) {
         ...
         //confList[i]->energy += eng.nonBondEnergy(nonBonds);
         confList[i] = NULL;
      } 
      if (stop == true) break;
   }
   ...
   if (clusterNum > 0) {
      string sNum = dynabase::setNum(clusterNum);
      text = "Generating " + sNum + " Clusters";
      lables.clear();  lables.push_back(text);
      e->setDisplayText(lables);
      e->sendEvent();
   ...
   if (min && !stop) {
      ...
      string method = "CG";
      for (int i=0; i<finNodes.size(); i++) {
         setMolCoords(tmpMol, coordHold);
         for (int j=0; j<finNodes[i]->angles.size(); j++) {
            bond *bd = rotBonds[j];
         ...
         //if (i % 10 == 0) {
            //string num; num.setNum(i);
            //string num2; num2.setNum(finNodes.size());
            string num = dynabase::setNum(i);
         ...
         molDB::writeSDF(outFile.c_str(), tmpMol, 1);
         string mol2Name = outFile + ".mol2";
         molDB::writeMol2(mol2Name.c_str(), tmpMol, 1);
         confCount++;
         if (stop == true) break;
      }
   }
   ...
   if (!min && !stop) {
     ...
     // e = new molChangeEvent(static_cast<QEvent::Type>(1010));
      for (int i=0; i<finNodes.size(); i++) {
         setMolCoords(tmpMol, coordHold);
         for (int j=0; j<finNodes[i]->angles.size(); j++) {
            bond *bd = rotBonds[j];
            float rotAngle = finNodes[i]->angles[j];
            //rotateBond(tmpMol, bd, rotAngle);
            rotateBond(bd, rotAngle);
         }
         //string num; num.setNum(i);
         //string num2; num2.setNum(finNodes.size());
         ...
         //lables[1] = label;
         lables.clear();  lables.push_back(label);
   ...
   //string total; total.setNum(confCount);
...
/*
void Conformation::run2(int num) {
   int ts =0; cin >> ts;
string del = "del " + outFile;
QFile f(outFile); f.remove();
   ...
   if (!min && !stop) {
     ...
     // e = new molChangeEvent(static_cast<QEvent::Type>(1010));
      for (int i=0; i<finNodes.size(); i++) {
         ...
         string num = dynabase::setNum(i);
         //string num2; num2.setNum(finNodes.size());
         //string num2 = setNum(finNodes.size());
         string num2 = dynabase::setNum(finNodes.size());
         string label = text + num + " of " + num2;
         lables[1] = label;
...
void Conformation::randomConf(string outFile, molecule *mol, int n, int clusterNum, bool min,float cutoff, float rmse,float minE, float maxForce, int maxIter) 
{
   list<molecule *> molList;
   molList.push_back(mol);
   
   vector<bond *> rotBonds;
   setRotableBonds(mol, rotBonds);
   setUp( &molList, rotBonds, outFile);
   eSetup(min, cutoff,  clusterNum, rmse, minE, maxForce, maxIter);
   run2(n);
}
*/
...
int Conformation::connectedAtoms(atom *atm, atom *other, vector<atom *> &atmsToRotate) {
   ...
   while (atomQueu.size() > 0) {
      ...
      atomQueu.pop_front();
      for (int i=0; i<currAtm->atoms.size(); i++) {
...
void Conformation::buildNonBondList(molecule *tmpMol, vector<nonBond *> &nonBonds, vector<nonBond *> &frozenNb) 
{
    ...
    *holdMol = *tmpMol;
    ...
    for (int i=0; i<tmpMol->nonBonds.size(); i++) {
       nonBond *nb = tmpMol->nonBonds[i];
       atom *atm1 = nb->atm1;
       atom *atm2 = nb->atm2;
       ...
       for (int j=0; j<frozenAtoms.size(); j++) {
...
void Conformation::buildTorsionList(vector<torsion *> &torsions, vector<torsion *> &frozenTors) 
{
   ...
   */
   ...
   for(int i=0; i<allMols.size(); i++) {
      ...
      for (int j=0; j<mol->torsions.size(); j++) {
...
float Conformation::nonBondCheck(vector<nonBond *> &nonbondVec, float cutoff) {
   float vdwE = 0;
   float chargeE = 0;	
   float tChargeE = 0;
   float tVanE = 0;
...
bool Conformation::setUp(list<molecule *> *molList, vector<bond *> &rotBonds, string outFile) {
  ...
  list<molecule *>::iterator mIter = molList->begin();
  fragment *frag = NULL;
  chain *ch = NULL;
  protein *prot = NULL;
  vector<bond *> newBonds;
  for (mIter; mIter != molList->end(); mIter++) {
     molecule *mol = *mIter;
     molecule *newMol;
     if (mol->rtti() <= 12) {
     ...
     if (mol->rtti() == 13) {
         ...
         *tmpChain = *ch;
         this->molList->push_back(tmpChain);
         molecule *lp = dynamic_cast<molecule *>(tmpChain->getParent());
         setNewBonds(newBonds, rotBonds, ch, tmpChain);
     }
     if (mol->rtti() == 14) {
         protein *prot = dynamic_cast<protein *>(mol);
         protein *tmpProt = new protein;
         *tmpProt = *prot;
         this->molList->push_back(tmpProt);
         setNewBonds(newBonds, rotBonds, prot, tmpProt);
     }
  }
  //this->molList = molList;
  //this->rotBonds = rotBonds;
...
bool Conformation::prepMolList(list<molecule *> &molList) {
   ...
   for (mIter; mIter != molList.end(); mIter++) {
      ...
      if (mol->rtti() <=12 && mol->atoms.size() < 200) {
         type = "GAFF";
         chrg = "AM1-BCC-fast";
      ...
      if (mol->rtti() >= 13 || mol->atoms.size() >= 200) {
         type = "AMBER_03";
         chrg = "AMBER";
         ff.setAtomTypes(mol, type);
         ff.buildLists(mol, type);
...
void Conformation::applyRot(bond *bd, vector<float> &fM) {
   ...
   for (int i=0; i<atomsToRotate[index].size(); i++) {
