...
descriptors::descriptors()
    : QMainWindow( 0, "descriptors", WDestructiveClose )
{
	QTextEdit *editor = new QTextEdit(NULL, "editor");
...
void descriptors::newDoc()
{
    descriptors *ed = new descriptors;
    ed->setCaption(tr("Qt Example - Application"));
    ed->show();
}

void descriptors::choose()
{
...
void descriptors::load( const QString &fileName )
{
    QFile f( fileName );
...
void descriptors::closeEvent( QCloseEvent* ce )
{
    if ( !e->isModified() ) {
	ce->accept();
	return;
