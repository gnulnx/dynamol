...
#ifndef DYNACOMPATOMTYPER_H
#define DYNACOMPATOMTYPER_H
...
public:
  ...
  *  It simply types the supplied rings atoms as opposed to finding all rings
  */
  void setAromatic(molecule *mol, vector<atom *> ring);
  ...
  bool isSulfonylBond(bond *bd);
  bool isAmidine(bond *bd);
  /** This return the number of heavy atoms connected to the atom */
  int  heavyAtoms(atom *atm);
  int  hydrogenAtoms(atom *atm);
  bool isDelocalized(bond *bd);
  bool isAtomDelocalized(atom *atm);  

  bool isDelocNitrogen(molecule *mol, atom *atm);
