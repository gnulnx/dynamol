...
energy::energy() 
{
   ...
   errorMsg = "";
   this->infoText = new vector<string>;
};

string energy::getErrorMsg()
{ return errorMsg; }
...
void energy::dieSetup(string method, float value) {
   ...
   //cout <<"energy.dieSetup " << dielectric << endl;
   //int test = 0;
   //cin >> test; 
}

void energy::setInfoText(vector<string> &infoText) {
...
//vector<double> energy::molEnergy(molecule *mol) {
   ...
   mol->potEnergy[4] = energies[4];
   mol->potEnergy[5] = energies[5];
   //setInfoEnergy(0);
   infoText = new vector<string>;
...
bool energy::setUpVectors(list<molecule *> &molList) {
   ...
   for (molIter = molList.begin(); molIter != molList.end(); molIter++) {
      molecule *mol = *molIter;
      for (unsigned int i=0; i<mol->angles.size(); i++) {
         //angle *ang = mol->angles[i];
         anglePtr ang = mol->angles[i];
   ...
   for (molIter = molList.begin(); molIter != molList.end(); molIter++) {
      molecule *mol = *molIter;
      for (unsigned int i=0; i<mol->torsions.size(); i++) {
   ...
   for (molIter = molList.begin(); molIter != molList.end(); molIter++) {
      molecule *mol = *molIter;
      for (unsigned int i=0; i<mol->nonBonds.size(); i++) {
         nbPtr nb = mol->nonBonds[i];
...
void energy::setAngles(vector<anglePtr> &Angles)  
{this->angleVec = Angles;};

void energy::setTorsions(vector<torPtr> &Torsions) 
{this->torVec = Torsions;};

void energy::setNonBonds(vector<nbPtr> &nonBonds)
...
bool energy::EandF(vector<float> &Energies) { 
	clearForces(); 
	setAngleBendForces();
	if (!setBondStretchForce()) {
		cout <<"Failed !setBondStretchForce"<<endl;
		return false;
	}
...
double energy::nonBondEandF_bsp(float maxDist) {
	return 0.0;
	//cout <<"Yep you are in nonBondEandF_bsp: " << maxDist << endl;
}
		...
		if (!atm1->frozen || !atm2->frozen) {
			...
			if (dist < 20.0) {
				...
				if (distDep) {
					...
					chargeE = 332.5*chrg * dist2;
				} else {
					cForce = 332.5*(chrg)*dist3 * dielectric;
					chargeE = 332.5*chrg * dist * dielectric;
				}
				double value = 24.0*epsilon*dist2*(2.0*(sigma12*dist12) -  (sigma6*dist6));
...
double energy::bondStretchEnergy() {
    ...
    for (unsigned int i=0; i<bondVec.size(); i++) {
    	bond *bd = bondVec[i];
...
double energy::bondStretchEnergy(vector<bond *> &bondVec) {
    float energy = 0;
    if (bondVec.size() == 0) return 0.0;
...
double energy::angleBendEnergy(vector<anglePtr> &angleVec) {
    double energy = 0;
    if (angleVec.size() == 0) return 0.0;
...
double energy::torsionalEnergy() {
    ...
    for (unsigned int i=0; i<torVec.size(); i++) {
...
double energy::torsionalEnergy(vector<torPtr> &torVec) {
    float energy = 0;
    if (torVec.size() == 0) return 0.0;
    for (unsigned int i=0; i<torVec.size(); i++) {
        torPtr tor = torVec[i];
...
float energy::nonBondEnergy(vector<nbPtr> &nonBondVec) {
   float vdwE = 0;
   float chargeE = 0;	
   float tChargeE = 0;
   float tVanE = 0;
...
bool energy::setBondStretchForce() {
	if (bondVec.size() == 0) 
		return false;
	...
	for (unsigned int i=0; i<bondVec.size(); i++) {
		...
		dist /= 1.0;
		double dedr = bd->K*(dist - bd->refLen);
		double Forces[3];
		...
		if (Forces[0] != Forces[0] || Forces[1] != Forces[1] || Forces[2] != Forces[2]) {
			cout <<"Force Fail(setBondStretch): " << Forces[0] << "\t"<<Forces[1]<<"\t"<<Forces[2]<<endl;
			return false;
		}
...
for (unsigned int i=0; i<angleVec.size(); i++) {
	...
	Forces1[0] = -DEDTheta*( cosTheta*bcDist*(cd1[0] - cd2[0]) 
                      + abDist*(cd2[0] - cd3[0]) ) * div1;

	Forces1[1] = -DEDTheta*( cosTheta*bcDist*(cd1[1] - cd2[1]) 
                      + abDist*(cd2[1] - cd3[1]) ) * div1;

	Forces1[2] = -DEDTheta*( cosTheta*bcDist*(cd1[2] - atm2->fCoords[2]) 
                      + abDist*(cd2[2] - cd3[2]) ) * div1;
	
	Forces3[0] = -DEDTheta*( cosTheta*abDist*(cd3[0] - cd2[0]) 
                      + bcDist*(cd2[0] - cd1[0]) ) * div3;

	Forces3[1] = -DEDTheta*( cosTheta*abDist*(cd3[1] - cd2[1]) 
                      + bcDist*(cd2[1] - cd1[1]) ) * div3;

	Forces3[2] = -DEDTheta*( cosTheta*abDist*(cd3[2] - cd2[2]) 
                      + bcDist*(cd2[2] - cd1[2]) ) * div3;
	
	double div2a = 1.0/(sinTheta*abDist*abDist*bcDist*bcDist);
	double div2b = 1.0/(sinTheta*abDist*bcDist);

	Forces2[0] = cosTheta*(abDist*abDist*(cd2[0] - cd3[0]) 
                     + bcDist*bcDist*(cd2[0]-cd1[0])) *div2a;
	
	Forces2[0] += ((cd1[0] - 2.0*cd2[0] + cd3[0])) * div2b;
	Forces2[0] *= -DEDTheta;
	
	Forces2[1] = cosTheta*(abDist*abDist*(cd2[1] - cd3[1]) 
                     + bcDist*bcDist*(cd2[1]-cd1[1])) * div2a;
	
	Forces2[1] += ((cd1[1] - 2.0*cd2[1] + cd3[1])) *div2b;
	Forces2[1] *= -DEDTheta;
	
	Forces2[2] = cosTheta*(abDist*abDist*(cd2[2] - cd3[2]) 
                + bcDist*bcDist*(cd2[2]-cd1[2])) * div2a;
	
	Forces2[2] += ((cd1[2] - 2.0*cd2[2] + cd3[2])) *div2b;
...
void energy::clearForces() {
   for (unsigned int i=0; i<atomVec.size(); i++) 
      atomVec[i]->zeroForces();
}
...
void energy::stochasticSearch(int maxIter) {
	...
	for (unsigned int i=0; i<mol->atoms.size(); i++) {
	   atom *atm = mol->atoms[i];
	   for (int j=0; j<3; j++) {
              atm->fCoords[j] = bestCoords[count]; 
              count++;
           }
	}
	}
...
float energy::SD_minimize(int maxIter, float rmsef, float maxForce, float minEDiff) {
	...
	double currE = 0;
		...
		//currE = energies[4];
		currE = energies[0];
		//cout <<currE <<"\t" << stepSize <<"\t" << dielectric << "\t" << iteration << endl;
		//int blahtest = 0;
		//cin >> blahtest;
		mol->potEnergy[0] = energies[0];
    		mol->potEnergy[1] = energies[1];
		...
		for (unsigned int j=0; j<atomVec.size(); j++) {
		   atom *atm = atomVec[j];
		   for (int k=0; k<3; k++) {
	...
	cout <<energies[0] <<", " << energies[1]<<","<<energies[2]<<","<<energies[3]<<","<<energies[4] <<", " << energies[5] << endl;
	return currE;
}
...
float energy::CG_minimize(int maxIter, float rmsef, float maxForce, float minEDiff) {
   ...
   for (unsigned int i=0; i<prevForces.size(); i++) {
      for (int j=0; j<3; j++) {
   ...
   for (iteration=0; iteration<maxIter; iteration++) { 
      ...
      EandF();
      currE = energies[0];
	  //cout <<"currE: "  << currE << "\t" << iteration<<endl;	
      //mol->potEnergy[0] = energies[0];
      //mol->potEnergy[1] = energies[1];
      //mol->potEnergy[2] = energies[2];
      //mol->potEnergy[3] = energies[3];
...
float energy::MC_fb(int maxIter, float temp) {
   file::writeSDF(outFile.c_str(), mol, 0);
   float stepSize = .0001;
...
float energy::MC(int maxIter, float temp) {  
   ...
   for (int i=0; i<maxIter; i++) {
	 ...
	 setInfoToEnergy(iteration);
      ...
      if (i % outIter == 0) {
        file::writeSDF(outFile.c_str(), mol, 1);
      }
      atomNum = rand() % atomVec.size();
      vector<float> cd(3);
      cd[0] = rand() % 100000000;
...
float energy::MC_torsional(int maxIter, float temp) {
   ...
   file::writeSDF(outFile.c_str(), rotBonds[0]->getParent(), 0);
   float rotInc = 1.0;
   EandF();
   float currE = energies[0];
   float prevE = currE;
...
void energy::RandVec3(vector<double> &vec) {
   ...
   while (s > 1.0) {
      int tmp1 = rand() % 10000; 
      double ran1 = tmp1;  ran1 /=  10000;
      int tmp2 = rand() % 10000; 
      double ran2 = tmp2;  ran2 /=  10000;
      x = 2.0*ran1-1.0;
      y = 2.0*ran2-1.0;
      s = x*x + y*y;
   }    
...
void energy::initVelocity(vector<double> &vel, double vMag) {
   vector<double> vSum(3);
   for (int i=0; i<3; i++) vSum[i] = 0.0;
   
   
   int numAtom = vel.size()/3; 
   //QString sNum;  sNum.setNum(numAtom);
   //QString error = "            " + sNum;
   int count = 0;
...
float energy::leapfrog() {
   ...
   for (int i=0; i<100000; i++) {
      ...
      //BSP PLAY
      vector<point *> Points(atomVec.size()); 
      //END BSP PLAY
      ...
      for (unsigned int j=0; j<atomVec.size(); j++) {
         ...
         float *aaa = new float;
         delete aaa;
         Points[j] = new point(atomVec[j]->getNum(), atomVec[j]->fCoords, 1.0);      
      ...
      //BSP PLAY STUFF
      BSP *bsp= new BSP(10, Points);
      delete bsp;

      for (unsigned int m=0; m<Points.size(); m++)
         delete Points[m];
     //END BSP PLAY STUFF
      KE = 0.5 * vSum/atomVec.size();
      avgKE += KE;
      ...
      if (i % 500 == 0) {
         ...
         string sPE =   "PE:    ";
         string sT    = "Time:  ";
         string nTemp = dynabase::setNum(avgTemp);
         string nKE = dynabase::setNum(avgKE);
         string nPE = dynabase::setNum(avgPE);
         string nT = dynabase::setNum(time);
         sTemp += nTemp; 
         sKE += nKE;
         sPE += nPE;
         sT += nT;
         infoText->clear();
         infoText->push_back( sT );
         infoText->push_back( sTemp ); 
         infoText->push_back( sKE );
         infoText->push_back( sPE );
...
float energy::MD() {
   ...
   for (int i=1; i<maxIter; i++) {
      ...
      //This is the NaN check....you had a hard time using a box here...seg fault && shit
      for (int i=0; i<4; i++) {
...
bool energy::setInfoToEnergy(int iteration) {
   ...
   if (iteration != -1) {
      ...
      tmp = dynabase::setNum(iteration);
      txt += tmp;
      infoText->push_back(tmp);
   
      txt = "Total Energy:    ";
      tmp = dynabase::setNum(energies[0]);
      txt += tmp;
      infoText->push_back(txt);

      txt = "Bond-Stretch:    ";
      tmp = dynabase::setNum(energies[1]);
      txt += tmp;
      infoText->push_back(txt);

      txt = "Angle-Bend:      ";
      tmp = dynabase::setNum(energies[2]);
      txt += tmp;
      infoText->push_back(txt);

      txt = "Torsion:            ";
      tmp = dynabase::setNum(energies[3]);
      txt += tmp;
      infoText->push_back(txt);

      txt = "Electrostatic:        ";
      tmp = dynabase::setNum(energies[4]);
      txt += tmp;
      infoText->push_back(txt);

      txt = "van der Waals:        ";
      tmp = dynabase::setNum(energies[5]);
      txt += tmp;
      infoText->push_back(txt);
